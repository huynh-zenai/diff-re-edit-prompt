...
### Jewelry pattern — Faceted gem ring with attached prongs

Reference summary:
> A silver ring with a raised colored gemstone, four prongs, and a small
> setting. The gem is faceted, not a smooth marble, and all hardware touches
> the band or stone.
...
const band = new THREE.Mesh(new THREE.TorusGeometry(0.24, 0.025, 14, 48), silverMat);
band.rotation.x = Math.PI / 2; // upright ring band, visible circular opening
root.add(band);

const setting = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.095, 0.040, 20), silverMat);
setting.position.set(0, 0.245, 0);
root.add(setting);

const gem = new THREE.Mesh(new THREE.OctahedronGeometry(0.085, 0), gemMat);
gem.scale.set(1.0, 0.75, 1.0);
gem.position.set(0, 0.315, 0);
root.add(gem);

// Four prongs overlap both setting and gem; none float beside the stone.
for (const [x, z] of [[-0.060, -0.060], [0.060, -0.060], [-0.060, 0.060], [0.060, 0.060]]) {
  const prong = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.008, 0.095, 8), silverMat);
  prong.position.set(x, 0.295, z);
  prong.rotation.x = z > 0 ? 0.18 : -0.18;
  prong.rotation.z = x > 0 ? -0.18 : 0.18;
  root.add(prong);
}

for (const x of [-0.055, 0.055]) {
  const shoulder = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.120, 10), silverMat);
  shoulder.rotation.z = Math.PI / 2;
  shoulder.position.set(x, 0.230, 0);
  root.add(shoulder);
}
```

Key idioms:
- Build the stable carrier first: band, setting, gem, then prongs.
- Use faceted low-poly geometry (`OctahedronGeometry`, `IcosahedronGeometry`,
  or a beveled/extruded cut) for gemstones before sparkle details.
- Prongs, bails, hooks, clasps, and chain links must overlap their parent
  slightly so the connection is visible.
- Keep metalness at or below 0.6; use material color to carry silver/gold.

### Food and dish pattern — Burger resting on a plate

Reference summary:
> A burger on a white plate. The burger has bun halves, visible filling
> layers, cheese, lettuce, and sesame seeds. The plate is support context and
> should not overpower the food.

Pattern to reuse:

```javascript
const root = new THREE.Group();
const plateMat = new THREE.MeshStandardMaterial({ color: 0xf3f0e8, metalness: 0.0, roughness: 0.38 });
const bunMat = new THREE.MeshStandardMaterial({ color: 0xd89b45, metalness: 0.0, roughness: 0.72 });
const pattyMat = new THREE.MeshStandardMaterial({ color: 0x5a2d1b, metalness: 0.0, roughness: 0.86 });
const cheeseMat = new THREE.MeshStandardMaterial({ color: 0xf2c84b, metalness: 0.0, roughness: 0.55 });
const lettuceMat = new THREE.MeshStandardMaterial({ color: 0x4f9f45, metalness: 0.0, roughness: 0.82 });
const seedMat = new THREE.MeshStandardMaterial({ color: 0xf5e3b0, metalness: 0.0, roughness: 0.7 });

const plate = new THREE.Mesh(new THREE.CylinderGeometry(0.31, 0.34, 0.026, 48), plateMat);
plate.position.y = -0.020;
root.add(plate);
const rim = new THREE.Mesh(new THREE.TorusGeometry(0.31, 0.012, 8, 48), plateMat);
rim.rotation.x = Math.PI / 2;
rim.position.y = -0.004;
root.add(rim);

function addLayer(name, radius, height, y, mat, scaleZ = 1.0) {
  const layer = new THREE.Mesh(new THREE.CylinderGeometry(radius, radius, height, 32), mat);
  layer.scale.z = scaleZ;
  layer.position.y = y;
  root.add(layer);
  return layer;
}

const lower_bun = addLayer("lower_bun", 0.170, 0.045, 0.025, bunMat, 0.92);
const patty = addLayer("patty", 0.165, 0.040, 0.070, pattyMat, 0.92);
const cheese = addLayer("cheese", 0.178, 0.010, 0.096, cheeseMat, 0.92);
const lettuce = addLayer("lettuce", 0.182, 0.014, 0.113, lettuceMat, 0.95);
const top_bun = addLayer("top_bun", 0.165, 0.070, 0.158, bunMat, 0.92);
top_bun.scale.y = 0.75;

for (let i = 0; i < 12; i++) {
  const a = i / 12 * Math.PI * 2;
  const r = i % 2 === 0 ? 0.070 : 0.115;
  const seed = new THREE.Mesh(new THREE.CapsuleGeometry(0.006, 0.014, 4, 8), seedMat);
  seed.position.set(Math.cos(a) * r, 0.202, Math.sin(a) * r * 0.75);
  seed.rotation.z = a;
  root.add(seed);
}
```
...
Key idioms:
- Model the edible object as the dominant silhouette; the plate is smaller,
  lower, and visibly supporting it.
- Preserve subtype cues with distinct layers, crust/rind/seed/filling marks,
  or cut faces before adding garnish.
- Food should touch the dish surface; do not leave a floating gap.
- Use separate materials for ceramic support and edible layers.
...
- Jewelry/gem carriers with attached prongs, bails, hooks, and faceted stones.
- Food objects supported by plates/bowls/trays without losing the edible
  object's silhouette.
