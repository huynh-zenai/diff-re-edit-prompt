...
class Pipeline:
    ...
    judge bracket when `coder_ensemble_size > 1`; iteration 1 then repairs
    that winner with two seeded variants and judges the repaired renders.
    Picks the best-scoring snapshot across iterations."""
    ...
    async def _run(self, task: PipelineTask) -> None:
        ...
        for iteration in range(self.max_iter + 1):
            ...
            if iteration == 0 and self.coder_ensemble_size > 1:
                ...
                iteration == 1
                and self.coder_ensemble_size > 1
                and last_report is not None
            ):
                # Repair the selected multigen winner with two seeded variants,
                # then judge the repaired renders against each other.
                await repair_winner_ensemble_stage(
                    task,
                    coder=self.coder,
                    judge=self.judge,
                    js_checker=self.js_checker,
            ...
            if (
                self._should_stop(report, iteration, task.best_score)
                and not force_seeded_repair
            ):
                break
