    ...
    while True:
        async with stage_guard(task, "coder", sem_coder, status):
            if task.iteration == 0 and attempt == 0:
                ...
                current_js = task.js_code
                if current_js is None:
                    raise RuntimeError("critic repair requested without current JS")
                ...
                current_js = task.js_code
                if current_js is None:
                    raise RuntimeError("checker repair requested without current JS")
...
async def repair_winner_ensemble_stage(
    task: PipelineTask,
    *,
    coder,
    judge,
    ...
    """Repair the current winner with multiple seeds, then judge the repairs."""
    if task.js_code is None:
        raise RuntimeError("repair ensemble requested without current JS")
    if task.rendered_png is None:
        raise RuntimeError("repair ensemble requested without current render")
    if task.image_bytes is None:
        raise RuntimeError("repair ensemble requested without reference image")

    osd = OSD.model_validate_json(task.osd) if task.osd is not None else None
    current_js = task.js_code
    current_render = task.rendered_png
    base_seed = task.seed + 10_000 + (task.iteration * repair_count)

    logger.info(
        f"[REPAIR ENSEMBLE] {task.stem} variants={repair_count} | "
        f"base_seed={base_seed} | score={last_report.overall_score:.2f}"
    )

    async def _repair_variant(idx: int) -> Candidate:
        actor = f"coder#repair{task.iteration}_{idx}"
        cand = Candidate(k=idx, seed=base_seed + idx)
        t0 = time.time()
        try:
            async with stage_guard(task, f"repair#r{idx}", sem_coder, status):
                cand.js_code = await coder.code_critic_repair(
                    task_id=task.stem,
                    osd=osd,
                    issues=last_report.issues,
                    overall_score=last_report.overall_score,
                    ...
                    image_bytes=task.image_bytes if coder_multimodal else None,
                    image_mime=task.image_mime,
                    render_png=current_render if coder_multimodal else None,
                    current_js=current_js,
            ...
            while True:
                shadow = PipelineTask(
                    stem=f"{task.stem}#repair{task.iteration}_{idx}",
                    image_url=task.image_url,
                    seed=cand.seed,
                )
                ...
                async with stage_guard(
                    task,
                    f"repair#r{idx}:checker",
                    sem_coder,
                    status,
                ):
                    cand.js_code = await coder.code_repair(
                        task_id=task.stem,
                        osd=osd,
                        js_errors=cand.js_errors,
                        current_js=cand.js_code or "",
                        actor_override=actor,
                        seed_override=cand.seed,
                    )
            ...
            cand.drop_reason = f"{exc.stage}:{type(exc.cause).__name__}"
        except Exception as exc:
            cand.drop_reason = type(exc).__name__

        cand.elapsed_s = time.time() - t0
        logger.info(
            f"[REPAIR ENSEMBLE] {task.stem} r{idx} | seed={cand.seed} | "
            f"valid={cand.js_valid} | drop={cand.drop_reason} | "
            f"elapsed={cand.elapsed_s:.1f}s"
        )
        return cand

    repaired = await asyncio.gather(*[
        _repair_variant(idx) for idx in range(repair_count)
    ])
    task.meta.setdefault("repair_ensembles", []).append({
        "iteration": task.iteration,
        "candidates": [
            {
                "r": cand.k,
                "seed": cand.seed,
                "js_valid": cand.js_valid,
                "drop_reason": cand.drop_reason,
    ...
    survivors = [
        cand for cand in repaired
        if cand.drop_reason is None and cand.rendered_png is not None
    ]
    if not survivors:
        logger.warning(
            f"[REPAIR ENSEMBLE] {task.stem} all repair variants failed; "
            "falling back to single repair path"
        )
        await code_and_check(
            ...
            coder_multimodal=coder_multimodal,
            status=status,
        ...
        for idx in range(repair_count):
            session_store.evict_actor(task.stem, f"coder#repair{task.iteration}_{idx}")
        return
    ...
    winner_actor = f"coder#repair{task.iteration}_{winner.k}"
    session_store.rename_actor(task.stem, winner_actor, "coder")
    for cand in repaired:
        actor = f"coder#repair{task.iteration}_{cand.k}"
        if actor != winner_actor:
            session_store.evict_actor(task.stem, actor)

    logger.info(
        f"[REPAIR ENSEMBLE] {task.stem} winner=r{winner.k} | "
        f"seed={winner.seed} | survivors={[c.k for c in survivors]}"
    )
